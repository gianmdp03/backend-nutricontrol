...
public class NotificationServiceImpl implements NotificationService {
  ...
  public Page<NotificationDetailDTO> listUserNotifications(User user, Pageable pageable) {
    Page<Notification> page =
        repository.findByUserAndIsReadFalseOrderByCreatedAtDesc(user, pageable);
    if (page.isEmpty()) {
      return Page.empty();
    }
  ...
  public void markAllAsRead(User user) {
    ...
    log.info(
        "Acción de usuario: Se marcaron como leídas {} notificaciones del usuario ID: {}",
        updatedCount,
        user.getId());
  }
  ...
  public void deleteAllNotifications(User user) {
    ...
    log.info(
        "Acción de usuario: Se eliminaron por completo las {} notificaciones del usuario ID: {}",
        deletedCount,
        user.getId());
  }
}
